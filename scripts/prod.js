...
import {
  GAME_PROD_FILE_REPLACEMENTS,
  OUTPUT_DIRECTORY_NAME,
  PRODUCTION_FILES_DIRECTORY_NAME,
  getFilesToCopy,
} from './shared';
...
const productionFileCopyPromises = GAME_PROD_FILE_REPLACEMENTS.map(
...
try {
  const allProductionFileCopiesPromise = Promise.all(
    productionFileCopyPromises,
  ).then(
    () => {
      console.log(
        `Successfully copied production files from ${PRODUCTION_FILES_DIRECTORY_NAME} to ${OUTPUT_DIRECTORY_NAME}`,
      );
    },
    (err) => {
      errorsFound = true;
  ...
  console.error(
    `Failed to copy production files from ${PRODUCTION_FILES_DIRECTORY_NAME} to ${OUTPUT_DIRECTORY_NAME}`,
  );
  throw err;
}
...
if (!errorsFound) {
  console.log(
    `Finished copying all game code files to ${OUTPUT_DIRECTORY_NAME}`,
  );
}
